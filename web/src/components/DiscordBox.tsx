...
export function DiscordBox({ discord, name }: DiscordProps) {
      ...
      >
        <div className="flex flex-col gap-1 justify-center items-center">
          <Dialog.Close onClick={() => setIsCopied(false)} title="Close">
            <X
              size={20}
              className="absolute top-5 right-5 text-zinc-400 hover:scale-110 hover:text-zinc-300 transition-all"
            />
          </Dialog.Close>
          ...
          <span className="text-zinc-400 mb-6">
            Domine todas suas partidas!
          </span>

          <span className="text-sm">Adicione {name} no Discord</span>
          <div className="flex flex-col w-full">
            <div
              className="flex justify-center items-center gap-3 bg-slate-800 py-3 rounded-md text-white font-medium relative cursor-pointer"
              title="Copiar"
            ...
            {copied && (
              <span className="text-sm text-green-400 mt-2 text-center animate-content-show">
