...
import { createAdFormSchema, weekDaysSchema } from "../validation/create-ad";
    ...
    <Dialog.Portal>
      ...
      >
        ...
        <form onSubmit={handleSubmit(handleCreateAd)} className="mt-8 flex flex-col gap-4">
          ...
          <Controller name="game" control={control} rules={{required: true}} render={({field}) => 
            <Select value={field.value} onChange={field.onChange} games={games} errorMessage={errors.game?.message}/>
          }/>
          <div className="flex flex-col gap-2 relative">
            <label htmlFor="name">Seu nome (ou nickname)</label>
            <Input id="name" type="text" {...register("name")} errorMessage={errors.name?.message}/>
          </div>

          <div className="grid grid-cols-2 gap-4 relative">
            <div className="flex flex-col gap-2">
              <label htmlFor="yearsPlaying">Joga a quantos anos?</label>
              <Input id="yearsPlaying" type="number" placeholder="Tudo bem ser zero :)" {...register("yearsPlaying")} errorMessage={errors.yearsPlaying?.message}
              />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor="discord">Qual o seu discord?</label>
              <Input id="Discord" type="text" placeholder="Usuario#0000" {...register("discord")} errorMessage={errors.discord?.message}/>
            </div>
          </div>
          ...
          <div className="flex gap-6">
            <div className="flex flex-col gap-2">
              ...
              <WeekdaysInput name="weekDays" control={control} errorMessage={errors.weekDays?.message}/>
            </div>
            <div className="flex flex-col gap-2 flex-1">
              <label htmlFor="hourStart">Qual horario?</label>
              <div className="grid grid-cols-2 gap-2 relative">
                <Input id="hourStart" type="time" placeholder="De" customClass="px-2" {...register("hourStart")} errorMessage={errors.hourStart?.message} className=""/>
                <Input id="hourEnd" type="time" placeholder="Até" customClass="px-2" {...register("hourEnd")} errorMessage={errors.hourEnd?.message}/>
              </div>
            </div>
          </div>
